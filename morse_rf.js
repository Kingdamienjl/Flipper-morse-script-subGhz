...
function send_morse() {
    let freq = parse_int(read_file(cfg_freq, "433920000"));
    let message = read_file(cfg_msg, "sos");
    let delay_ms = parse_int(read_file(cfg_delay, "60"));
    let repeat = parse_int(read_file(cfg_repeat, "0"));

    print("Msg: " + message + " @ " + to_string(freq) + "Hz");
